...
export default function EditTable({
  ...
  useEffect(() => {
    if (data) {
      const { capacity, status, transport } = data.payload.data;
      form.reset({
        capacity,
        status,
        transport,
            ...
            <div className="grid gap-4 py-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <div className="grid grid-cols-4 items-center justify-items-start gap-4">
                      <Label htmlFor="description">Vị trí </Label>
                      <div className="col-span-3 w-full space-y-2">
                        <Select
                          onValueChange={field.onChange}
                          value={field.value}
                        >
                          ...
                          <SelectContent>
                            {TransportStatusValues.map((transport) => (
                              <SelectItem key={transport} value={transport}>
                                {getVietnameseTransStatus(transport)}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
