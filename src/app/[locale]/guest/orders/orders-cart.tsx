...
export default function OrdersCart() {
  ...
  //   if (!address.trim()) {
  //     toast({ description: "Cập nhật địa chỉ nhận hàng" });
  //     return;
  //   }
  ...
  const handlePaymentDialogOpen = async () => {
    ...
    const phone = guestPhoneData?.payload?.data?.phone?.trim();
    ...
    try {
      const guestInfo = await guestApiRequest.getGuest(guestPhoneData);
      setguestInfo(guestInfo);

      const addr = guestInfo?.payload?.data?.address || "";
      setAddress(addr);
  ...
  const { waitingForPaying, paid } = useMemo(() => {
    return orders.reduce(
      (result, order) => {
        const totalPrice = order.dishSnapshot.price * order.quantity;
        if (
          order.status === OrderStatus.Delivered ||
          order.status === OrderStatus.Processing ||
          order.status === OrderStatus.Pending
        ) {
          result.waitingForPaying.price += totalPrice;
          result.waitingForPaying.quantity += order.quantity;
        } else if (order.status === OrderStatus.Paid) {
          result.paid.price += totalPrice;
          result.paid.quantity += order.quantity;
        }
        return result;
      },
      {
  ...
  const handleOnlinePayment = async () => {
    try {
      ...
      toast({
        description: `Thanh toán online thất bại: ${(error as Error).message}`,
      });
    }
  };
